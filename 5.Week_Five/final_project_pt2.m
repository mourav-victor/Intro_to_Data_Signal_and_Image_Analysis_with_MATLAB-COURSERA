...
for i=1:length(centroid)
    k=msk_dil_erd(centroid(i,2)-filtsizeh:centroid(i,2)+filtsizeh,centroid(i,1)-filtsizeh:centroid(i,1)+filtsizeh); 
    k=k(:)
    D(i,1)=corr(dimefilter(:),k); 
    D(i,2)=corr(nickelfilter(:),k); 
    D(i,3)=corr(quarterfilter(:),k); 
end
